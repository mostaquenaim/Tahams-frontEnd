...
const UpdateCategory = ({ item }) => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const checkAvailability = categories.some(
        (category) => category.categoryName === updatedData.categoryName,
      );
    ...
    <>
      ...
      <div className="flex flex-col justify-center items-center text-center bg-gradient-to-b from-zinc-50 to-blue-100 h-screen">
        <div className="p-5 bg-white shadow-md w-96 flex flex-col gap-3 rounded-lg">
          ...
          <div>
            <Link href={`change-category-image/${item.id}`}>Update Image</Link>
          </div>
          <div className="my-2">
...
export async function getServerSideProps(context) {
  ...
  const response = await axios.get(
    `${process.env.NEXT_PUBLIC_API}/admin/getCategoryById/${id}`,
  );
