...
const AddNewArrivals = ({ previousArrivals }) => {
    ...
    const [formData, setFormData] = useState(
        [...previousArrivals, ...Array(Math.max(0, 4 - previousArrivals.length)).fill({
            name: '',
            description: '',
            category: '',
            subSubCategory: '',
    ...
    const handleChange = (index, e) => {
        ...
        setFormData((prevState) => {
            ...
            updatedFormData[index] = {
                ...
                preview: name === 'filename' ? URL.createObjectURL(files[0]) : updatedFormData[index].preview,
            };
                        ...
                        <div className="w-full md:w-1/4 relative">
                            {!isEditing[index] ? (
                                <div className="flex items-center justify-between border p-2 rounded">
                                    <span>
                                        {item.subsub.name || "Select Sub-Subcategory"}
                                    </span>
                                    <button
                                        onClick={() => {
