...
export default function AddProduct() {
  ...
  // Development only (see DevFillButton). Fills every required field, picks
  // one existing product type, and generates the images.
  const fillRandom = async () => {
    const name = randomLabel();
    const buyingPrice = randInt(200, 1200);
    setForm({
      name,
      serialNo: String(randInt(1000, 9999)),
      note: 'Generated by the dev random fill',
      vatPercentage: '0',
      discountPercentage: String(pick([0, 0, 5, 10, 15, 20])),
      buyingPrice: String(buyingPrice),
      sellingPrice: String(buyingPrice + randInt(150, 800)),
      description: randomSentences(1),
      longDescription: randomSentences(3),
    });
    if (colors.length > 0) setColor(pick(colors).name);
    setTags(['cloth', 'dev']);

    if (productTypes.length > 0) {
      const type = pick(productTypes);
      const hasSizes = sizes.length > 0 && Math.random() < 0.7;
      const quantities = {};
      if (hasSizes) {
        sizes.forEach((size) => {
          quantities[size.id] = String(randInt(0, 20));
        });
        // The form needs at least one size with stock to be worth submitting.
        quantities[pick(sizes).id] = String(randInt(5, 20));
      } else {
        quantities.none = String(randInt(5, 30));
      }
      setSelection({ [type.id]: { hasSizes, quantities } });
    }

    setFeatured([await randomImageFile(name)]);
    setAdditional(
      await Promise.all(
        Array.from({ length: randInt(0, 2) }, () => randomImageFile(name)),
      ),
    );
    setErrors({});
    setFormError('');
  ...
  return (
    <AdminPage title="Add product" width="narrow">
      <PageHeader
        ...
        actions={<DevFillButton onFill={fillRandom} />}
      />
